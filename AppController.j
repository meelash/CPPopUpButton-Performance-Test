...
@implementation AppController : CPObject
{
    ...
    @outlet MHPopUpButton testButton2;
}

- (void)applicationDidFinishLaunching:(CPNotification)aNotification
{
    [self populateButton:nil];
//    [testButton2 initWithFrame:CPRectMake(42, 185, 132, 34)];
...
- (@action)removeAllItems:(id)sender
{
    [testButton2 removeAllItems];
}

- (@action)addItems:(id)sender
{
    [testButton2 addItemsWithTitles:TEST_ARRAY1];
}

- (@action)changedItem:(id)sender
{
    alert([testButton2 indexOfSelectedItem]);
}
